...
function endsWith(str, substr) {
  if (typeof str !== 'string' || typeof substr !== 'string') {
    throw new TypeError('Both arg must be string');
  }
...
function formatTime(minutes, seconds) {
  if (
    ...
    throw new Error('Both minutes and seconds must be non-negative numbers');
  }
...
function getCardId(value) {
  ...
  const suits = ['♣', '♦', '♥', '♠'];
  const rank = value.startsWith('10') ? '10' : value[0]; // Handle '10' as a special case
  const suit = value.slice(-1); // Extract the suit from the last character
  const rankIndex = ranks.indexOf(rank); // Find the rank's index
  const suitIndex = suits.indexOf(suit); // Find the suit's index
  return suitIndex * 13 + rankIndex; // Calculate the zero-based index
}
